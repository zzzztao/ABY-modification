...
share* Binary_exponent_Float(uint32_t bitlen, uint32_t dp, share* s_exponent, BooleanCircuit* circ){
	...
	float f_Decimal_base[10] = {1.648721270,1.284025417,1.133148453,1.064494459,1.031743407,1.015747709,1.007843097,
				   1.003931389,1.001955034,1.000977039};//{e^1/2,e^1/4,e^1/8,e^1/16,e^1/32,e^1/64}

	uint32_t Integer_base[4], Decimal_base[10];
	for(int i=0;i<4;i++){
		Integer_base[i] = *((uint32_t*) &f_Integer_base[i]);
		//cout<<"Integer_base["<<i<<"] is "<<fixed<< setprecision(5) <<*(float*) &Integer_base[i]<<endl;
	}
	for(int i=0;i<10;i++)
		Decimal_base[i] = *((uint32_t*) &f_Decimal_base[i]);
		//cout<<"test3["<<i<<"] is "<<fixed<< setprecision(5) <<*(float*) &test3[i]<<endl;
	share* s_Integer_base = circ->PutSIMDINGate(4, Integer_base, bitlen, SERVER);
	share* s_Decimal_base = circ->PutSIMDINGate(10, Decimal_base, bitlen, SERVER);
	share* s_out;
...
share* Exponent_Pos(uint32_t bitlen, uint32_t decimalplace, share* s_exponent, share* s_Integer_base, share* s_Decimal_base, BooleanCircuit* circ){
	...
	for(int i = 0;i<4;i++){
		...
		in_result = circ->PutFPGate(in_result, temp, MUL, bitlen);//类乘得到整数部分的最终结果
	}
...
share* Exponent_Neg(uint32_t bitlen, uint32_t decimalplace, share* s_exponent, share* s_Decimal_base, BooleanCircuit* circ){
	...
	for(int i = 0;i<10-decimalplace;i++){
