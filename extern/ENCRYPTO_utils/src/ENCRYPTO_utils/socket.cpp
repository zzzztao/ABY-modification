...
bool CSocket::Connect(const std::string& host, uint16_t port) {
	...
	//std::cout << "connect address:" << host << " " << " port " << port << std::endl;
...
size_t CSocket::Receive(void* buf, size_t bytes) {
	...
	unsigned char *p;
	p = (unsigned char *)buf;
	receive_out.open("receive_out.txt", std::ios::out | std::ios::app);
...
size_t CSocket::Send(const void* buf, size_t bytes) {
	...
	unsigned char *p;
	p = (unsigned char *)buf;
	send_out.open("send_out.txt", std::ios::out | std::ios::app);
	for(int i = 0; i < bytes; i++){
		send_out << *p ;
		p++;
	}
